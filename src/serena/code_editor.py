...
class CodeEditor(Generic[TSymbol], ABC):
    ...
    def _save_edited_file(self, edited_file: "CodeEditor.EditedFile") -> None:
        ...
        notify_known = getattr(self.project, "ls_notify_file_changed", None)
        if callable(notify_known):
            # Native V8 Project owns semantic invalidation + targeted LSP sync.
            notify_known(edited_file.relative_path)
        else:
            # Compatibility for older Project-like adapters/tests.
